  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {isLoading && (
          <div className="glass col-span-full rounded-2xl border border-white/10 p-5 text-sm text-muted-foreground">
            Загружаем отзывы…
          </div>
        )}
        ...
        {reviews.map((r) => (
          <article key={r.id} className="glass rounded-2xl border border-white/10 p-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-3">
                <div className="font-semibold text-foreground">{r.author_name}</div>
                {r.is_demo && (
                  <span className="rounded-sm border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-muted-foreground">
                    образец
                  </span>
                )}
              </div>
